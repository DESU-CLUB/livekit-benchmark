...
EVENTS_LOG = Path("/home/user/livekit-webhook/events.log")
SERVER_PORT = 4000
...
def test_script_imports_livekit_server_sdk():
    ...
    assert "livekit-server-sdk" in source, (
        "server.js must import from 'livekit-server-sdk'"
    )
...
def test_script_uses_express_raw_middleware():
    ...
    assert "express.raw" in source, (
        "server.js must use express.raw() middleware for raw body parsing"
    )
...
def test_script_defines_webhook_post_route():
    source = SCRIPT_FILE.read_text()
    assert "/webhook" in source, "server.js must define a POST /webhook route"
